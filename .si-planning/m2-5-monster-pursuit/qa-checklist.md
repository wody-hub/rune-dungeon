...
- [ ] C-6 `[STEP]`: `returning` 슬라임을 한 스텝 이내에 두고 플레이어를 스폰에서 `3.0` 미만(예: `0.1`)에 배치한다. 첫 `dt=1/60` 틱은 정확한 스폰·`idle`, 두 번째 틱부터만 `engaged` 또는 `chasing`으로 재감지한다.
...
- [ ] E-1 `[LIVE]`: 시간 측정 기준은 더블클릭이 아니라 플레이어가 무기 범위 `1.8` 안에 들어와 `mode=attacking`, `pendingHitMs=320`으로 전환된 틱이다. 이 기준에서 논리 히트 시점은 첫 `320ms`, 두 번째 `1570ms`(`1250ms` 모션 + 다음 `320ms` 히트 프레임)이며 60Hz 화면 관측에는 최대 1프레임(`16.7ms`) 지연을 허용한다. 각 허용 구간 전에는 HP가 감소하지 않고 이후에는 정확히 한 번씩 감소하며 대상 HUD의 HP 값과 바가 내부 상태와 일치한다.
