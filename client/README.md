...
4. **입력은 Intent 큐.** 입력을 즉시 실행하지 않고 `GameIntent` 판별 유니온으로 큐에 넣어 프레임 틱에서 소비한다. 시뮬레이션에는 지면 이동, 몬스터 타깃 선택, 자동공격 전환 variant가 있으며, 현재 씬은 지면 클릭만 큐잉한다. 몬스터를 클릭해 타깃 intent를 만드는 씬 입력은 M2 시각 연결에서 추가한다.
